...
import { Request, Response } from 'express'; // <-- Import des types Express nécessaires
...
export class RolesController {
  ...
  async becomeOrganizer(@Req() req: Request, @Res() res: Response) {
    ...
    try {
      ...
      return res.status(HttpStatus.OK).json({ message: 'Rôle mis à jour vers organisateur' });
    } catch (err: any) {
